{
  ...
  Fitter.SetUp().LoadVariable("KpKmCosTh[-1,1]");
  Fitter.SetUp().LoadVariable("KpKmPhi[-3.14159,3.14159]");
  Fitter.SetUp().LoadVariable("KpKmPolPhi[-3.14159,3.14159]");
  Fitter.SetUp().LoadVariable("KpKmPol[0.7,0.8]");
  ...
  ComponentsPdfParser  parser=HS::FIT::PolarisedSphHarmonicMoments("SphHarmonicMoments","KpKmCosTh","KpKmPhi","KpKmPolPhi","KpKmPol",4,0,4);
  ...
  sum +=       "+ SUM(L[1|4],M[0|4<L+1]){H_0_L_M[0,-1,1]*K_L*ReY_L_M(KpKmCosTh,KpKmPhi,Y_L_M)}";
  sum +=       "+ SUM(L[0|4],M[0|4<L+1]){H_1_L_M[0,-1,1]*K_L*ReY_L_M(KpKmCosTh,KpKmPhi,Y_L_M)*COS2PHI}";
  sum+=        "+ SUM(L[1|4],M[1|4<L+1]){H_2_L_M[0,-1,1]*K_L*ImY_L_M(KpKmCosTh,KpKmPhi,Y_L_M)*SIN2PHI}";
  ...
  Fitter.LoadData("FINALOUTTREE","/u/home/matthewn/Chanser/Final_States/KpKm/Output_fall2018_skim3_Outbending_290921_correct_branches/matthewn/KpKm_ALLALL_configuration1/FinalState.root");
  Fitter.LoadSimulated("FINALOUTTREE","/u/home/matthewn/Chanser/Final_States/KpKm/Output_Simulated_3723_noFTcorrection_221021/matthewn/KpKm_ALLALL_configuration1/FinalState.root","SphHarmonicMoments");
